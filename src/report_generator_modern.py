...
def _create_modern_language_chart(languages, path):
    ...
    try:
        ...
        legend_labels = [f"{lang['name']}: {lang['count']:,} reseñas ({lang['percentage']}%)" for lang in languages]
...
def create_modern_report(df, client_name, output_path, cfg, project_root="."):
    ...
    try:
        ...
        _create_languages_table_modern(doc, cfg.get("languages", []))
        ...
        # Análisis IA
        analysis_heading = doc.add_paragraph("🤖 Análisis Inteligente")
        analysis_heading.style = doc.styles['Heading 2']
        analysis_heading.runs[0].font.color.rgb = PRIMARY
        
        # Crear párrafos del análisis IA con mejor formato
